  ...
  if (iren != NULL && event == vtkCommand::KeyPressEvent) {
    if (iren->GetKeyCode() == 'a')
      ...
      this->_showVoxels(thresholdPIDEntry->GetValue(), false);
		...
		this->_savePolypScene(thresholdPIDEntry->GetValue());
  }
  // apply button
...
/**
 * Save polyp surface w/ or wo/ voxels into a VTK data file.
 */
void vtkSlicerVirtualColonGUI::_savePolypScene(const char* polypId_)
{
  vtkSmartPointer<vtkKWFileBrowserDialog> dlg = vtkSmartPointer<vtkKWFileBrowserDialog>::New();
  dlg->SetApplication(this->GetApplication());
  dlg->Create();
  dlg->SetSaveDialog(1);
  //  dlg->ChooseDirectoryOn();
  dlg->SetTitle(this->GetApplication()->GetName());
